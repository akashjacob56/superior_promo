...
//        Schema::defaultStringLength(191);
        ...
        view()->composer('*', function ($view) {
            ...
            $selected_language = Language::query()
                ->where('status_id', 1)
                ->with('default_language_translation')->with([
                    'language_translation' => function ($query) use ($language_id) {
            ...
            \Illuminate\Support\Facades\App::setLocale($language_code);
            ...
            $menu_categories = Category::where('status_id', '1')
                ->where('is_parent_category', 1)
                ->with('default_category_translation')
                ->with([
                ...
                ->with([
                    "child_categories" => function ($query) use ($language_id) {
                        $query->orderBy('updated_at', 'desc')
                            ->with([
                                'category' => function ($query) use ($language_id) {
                                    $query
                                        ->with('default_category_translation')->with([
                                            'category_translation' => function ($query) use ($language_id) {
                                                $query->where('language_id', $language_id)->orderBy('updated_at',
                                                    'desc');
                                            }
                                        ]);
                                }
                            ...
                            ->with([
                                "sub_child_categories" => function ($query) use ($language_id) {
            ...
            if (Auth::user()) {
                ...
                $permissions = Permission::orWhereIn('permission_id', $role_permission_ids)->orWhereIn('permission_id',
                    $user_permission_ids)->pluck('permission_name');
            }
            ...
            if (Auth::user()) {
                ...
                $notifications = Notification::with('default_notification_translation')->with([
                    'notification_read' => function ($query) use ($user_id) {
                        $query->where('user_id', $user_id);
                    }
                ])->with('notificationclass')->with([
                    'product_section' => function ($query) {
                        $query->with('default_product_translation');
                    }
                ])->with([
                    'category_section' => function ($query) {
                        $query->with('default_category_translation');
                    }
                ])->orderBy('updated_at', 'desc')->where('status_id', 1)->where("created_at", '>=',
                    $user->created_at)->take(3)->get();

                $notification = Notification::with('default_notification_translation')->whereDoesntHave('notification_read',
            ...
            $active_languages = Language::where('status_id', 1)->with('default_language_translation')->with([
                'language_translation' => function ($query) use ($language_id) {
                    $query->where('language', $language_id);
                }
            ])->get();
            ...
            $about = About::with('default_aboutus_translation')->with([
                'aboutus_translation' => function ($query) use ($language_id) {
                    $query->where('language_id', $language_id);
                }
            ])->first();
            $term_condition = TermCondition::with('default_term_condition_translation')->with([
            ...
            View::share('base_url', route('home'));
            ...
            $countries = Country::where('country_id', '!=', 1)->where('status_id', '=',
                1)->with('default_country_translation')->get();
            ...
            $new_sections = Section::where('section_id', '!=', 1)->with('section_products')->where('status_id',
                1)->orderBy('section_id', 'desc')->take(2)->get();
            $theme_path = "theme1";
