...
class PublicController extends Controller
{
    ...
    public function getBlogDetails(Request $request)
    {
        ...
        return view('superior.blog-details')->with('blog', $blog)->with('five_blogs', $five_blogs)->with('blogs',
            $blogs)->with('seo_title', $seo_title);
    ...
    public function getMyAccSelectOrderHistoryDays(Request $request)
    {
        ...
        $order_history = Order::where('is_reorder', 0)->where('user_id',
            $user_id)->with('orderitems')->where('created_at', '>', now()->subDays($days)->endOfDay())->orderBy('id',
            'desc')->get();
        ...
        if ($order_history != "[]") {
            ...
            $view = View::make('superior.select-order-history')->with('order_history',
                $order_history)->with('today_current_date', $today_current_date);
    ...
    public function SearchBarHeader(Request $request)
    {
        ...
        $product_translation = ProductTranslation::with('product')->where('product_name', 'like',
            '%'.$keyword.'%')->orWhere('product_id', 'like', '%'.$keyword.'%')->take(5)->get();


        $product_ids = ProductTranslation::with('product')->where('product_name', 'like',
            '%'.$keyword.'%')->orWhere('product_id', 'like', '%'.$keyword.'%')->take(5)->pluck('product_id');
        ...
        if ($category_ids != "") {
            $category_translation = CategoryTranslation::with('category_product')->whereIn('category_id',
                $category_ids)->take(10)->get();
        } else {
        ...
        $data = array(
            "category_translation" => $category_translation, "product_translation" => $product_translation,
            "products" => $products
        );
    ...
    public function getMyAccSearchOrderHistory(Request $request)
    {
        ...
        if ($order_id != "") {
            $order_history = Order::where('id', $order_id)->where('is_reorder', 0)->where('user_id',
                $user_id)->with('orderitems')->get();
        } else {
            $order_history = Order::where('is_reorder', 0)->where('user_id',
                $user_id)->with('orderitems')->orderBy('id', 'desc')->get();
        }
        ...
        if ($order_history != "[]") {
            ...
            $view = View::make('superior.search-order-history')->with('order_history',
                $order_history)->with('today_current_date', $today_current_date);
            $sections = $view->renderSections();
    ...
    public function getEditCartItem(Request $request)
    {
        ...
        $old_cart_item_data = CartItems::with('product')->with('cartitemcolor')->with('cartitemimprint')->where('id',
            $cart_item_id)->first();


        $CartItemProductOptions = CartItemProductOptions::with('cartitemSubproductOptions')->where('cart_item_id',
            $cart_item_id)->get();
        ...
        $product_categorydata = CategoryProduct::where('product_id',
            $product_id)->with('product_translation')->with('category_link')->first();
        ...
        if ($product_colors != "") {
            ...
            $product_sub_images = ProductColor::where('product_color_group_id',
                $product_color_group_id)->with('color')->get();
        ...
        $related_products = Product::whereIn('product_id',
            $category_product_ids)->with('default_product_translation_full')->with('skus')->with('product_prices')->with('product_color_group')->take(10)->get();
        ...
        /*for all data */
        $data = array(
            "product" => $product, "productdetails" => $productdetails, "all_color" => $all_color,
            "product_sub_images" => $product_sub_images, "product_pricing" => $product_pricing, "imprints" => $imprints,
            "all_product_images" => $all_product_images, "get_product_colors" => $get_product_colors,
            "product_review" => $product_review, "related_products" => $related_products,
            "lower_price_products" => $lower_price_products, "product_option" => $product_option,
            "product_categorydata" => $product_categorydata, "child_category" => $child_category,
            "parent_category" => $parent_category, "product_Apparels" => $product_Apparels,
            "old_cart_item_data" => $old_cart_item_data, "CartItemProductOptions" => $CartItemProductOptions,
            "CartItemSizeGroups" => $CartItemSizeGroups, "CartItemArtProofs" => $CartItemArtProofs
        );
    ...
    public function getlocationsCountryCheckout(Request $request)
    {
        ...
        $states = State::where('country_id', $country_id)->where('state_id', '!=', 1)->where('status_id',
            1)->with('state_translation')->get();
    ...
    public function postImprintOptionSelect(Request $request)
    {
        ...
        $view = View::make('superior.imprint-add-color-btn')->with('imprint_id', $imprint_id)->with('imprint_colors',
            $imprint_colors)->with('max_colors', $max_colors);
        $sections = $view->renderSections();

        $data = array(
            "sections" => $sections['imprint-add-color-btn-section'], "imprint_id" => $imprint_id,
    ...
    public function postReorder(Request $request)
    {
        ...
        if ($request->exactly_same_no_checkbox == "true") {
            ...
            if ($request->item_color_going_to_change_yes == "true") {
                ...
                if ($order_item != "") {
                    ...
                    if ($cart_item_colors != "") {
                        $product_color = ProductColor::where('product_color_group_id',
                            $cart_item_colors->product_color_group_id)->where('id',
                            $cart_item_colors->product_color_id)->where('color_id',
                            $cart_item_colors->color_id)->first();
                        if ($product_color != "") {
                ...
                if ($order_item != "") {
                    ...
                    if ($cart_item_imprints != "[]") {
                        foreach ($cart_item_imprints as $cart_item_imprint) {
                            ...
                            $cart_item_imprint_colors = CartItemImprintColors::where('cart_item_imprint_id',
                                $cart_item_imprint->id)->get();
            ...
            if ($order_item != "") {
                ...
                if ($cart_item_colors != "") {
                    $product_color = ProductColor::where('product_color_group_id',
                        $cart_item_colors->product_color_group_id)->where('id',
                        $cart_item_colors->product_color_id)->where('color_id', $cart_item_colors->color_id)->first();
                    if ($product_color != "") {
                        $main_selected_color = $product_color->color_id;
                    }
                } else {
        ...
        if ($request->shipp_addr_ord_same_as_befr_checkbox == "true") {
            ...
            $user_id = $this->getLoginUserId();
            $address = Address::orderBy('address_id', 'desc')->where('user_id', $user_id)->where('is_shipping',
                1)->where('is_default_add', 1)->first();
        ...
        if ($request->bill_addr_ord_same_as_befr_checkbox == "true") {
            ...
            $user_id = $this->getLoginUserId();
            $address = Address::orderBy('address_id', 'desc')->where('user_id', $user_id)->where('is_billing',
                1)->first();
...
// echo $artwork_path;die;
        ...
        if ($imprint_ids != "[]") {
            ...
            for ($i = 0; $i < count($imprint_ids); $i++) {
                ...
                $imprint_price_quantities = ImprintPrice::where('imprint_id', $imprint_id)->where('quantity', '<=',
                    $quantity_cart)->pluck('quantity');
                ...
                $get_imprint_price = ImprintPrice::where('imprint_id', $imprint_id)->where('quantity',
                    $closest)->first();
        ...
        if ($main_selected_color != "") {
            ...
            $product_color_data = ProductColor::where('color_id',
                $selected_color)->with('color_group')->with('color')->first();
        ...
        $Product_prices = ProductPrice::where('product_id', $product_id)->where('count_from', '<=',
            $quantity)->pluck('count_from');
        $closest = null;
        ...
        $cart_item_new = CartItems::with('product')->with('cartitemcolor')->with('cartitemimprint')->where('id',
            $new_cart_item_id)->first();
    ...
    public function PostSuboptionQuantity(Request $request)
    {
        ...
        $Product_suboption_prices = ProductSubOptionPrices::where('product_sub_option_id',
            $sub_option_id)->where('quantity', '<=', $quantity_cart)->pluck('quantity');
        ...
        if ($Product_suboption_prices != "[]") {
            ...
            $matches_quantitydata = ProductSubOptionPrices::where('product_sub_option_id',
                $sub_option_id)->where('quantity', $closest)->first();
...
return $data;*/
    ...
    public function PostCartDiscount(Request $request)
    {
        ...
        $discount_data = Discount::with('discount_types')->with('discount_apply')->with('discount_customer_apply')->where('discount_code',
            $discount_code)->first();
        ...
        if ($discount_data->customer_eligibility_id == 1) {
            ...
            $discount_customer_apply = DiscountCustomerApply::where('discount_id',
                $discount_data->discount_id)->pluck('customer_id');
    ...
    public function getPromocode(Request $request)
    {
        ...
        $promocodes = Discount::with('discount_types')->with('discount_apply')->with('discount_customer_apply')->where('discount_code',
            $promocode)->first();
        ...
        if ($promocodes->only_one_use_per_customer == 1) {
            $discount_applied = DiscountApplied::where('user_id', $user_id)->where('discount_id',
                $promocodes->discount_id)->first();
            if ($discount_applied != "") {
        ...
        $user_id = $this->getLoginUserId();
        $discount = "";
        $carts = Cart::where("user", $user_id)->with('user')->with([
            'sku' => function ($query) use ($language_id) {
                $query->with([
                    'product' => function ($query) use ($language_id) {
                        $query->with('default_product_translation')->with([
                            'product_translation' => function ($query) use ($language_id) {
                                $query->where('language_id', $language_id);
                            }
                        ])->with([
                            'brand' => function ($query) use ($language_id) {
                                $query->with('default_brand_translation')->with([
                                    'brand_translation' => function ($query) use ($language_id) {
                                        $query->where('language_id', $language_id);
                                    }
                                ]);
                            }
                    ...
                    'parent_variant' => function ($query) use ($language_id) {
                        $query->with('default_variant_option_translation')->with([
                            'variant_option_translation' => function ($query) use ($language_id) {
                                $query->where('language_id', $language_id);
                            }
                        ]);
                    }
                ])->with([
                    'child_variant' => function ($query) use ($language_id) {
                        $query->with('default_variant_option_translation')->with([
                            'variant_option_translation' => function ($query) use ($language_id) {
                                $query->where('language_id', $language_id);
                            }
        ...
        if ($promocodes->customer_eligibility_id == 1) {
            ...
            $discount_customer_apply = DiscountCustomerApply::where('discount_id',
                $promocodes->discount_id)->pluck('customer_id');
            if ($discount_customer_apply->contains($user_id)) {
                $this->response['msg'] = $promocode_applied_successfully;
                $this->response['status'] = "true";
                $this->response['promocode'] = $promocodes->discount_code;
    ...
    public function postReorderRequestExactlySameContentChangeNo(Request $request)
    {
        ...
        if ($product_color_group != "") {
            ...
            $product_colors = ProductColor::where('product_color_group_id',
                $product_color_group_id)->with('color')->get();
        ...
        $view = View::make('superior.reord-req-ext-same-cnt-chnge-no')->with('product_prices',
            $product_prices)->with('product', $product)->with('product_colors', $product_colors)->with('imprints',
            $imprints)->with('order_id', $order_id);
        $sections = $view->renderSections();
            ...
            if ($response != null) {
                ...
                if ($response->getMessages()->getResultCode() == "Ok") {
                    ...
                    if ($tresponse != null && $tresponse->getMessages() != null) {
                        ...
                        $profile_response = "";
                        $data = array(
                            'message_text' => $message_text, 'msg_type' => $msg_type, 'is_status' => $is_status,
            ...
            if ($cim_no == "") {
                if ($is_status == "Success") {
                    ...
                    if ($response != null && $response->getMessages()->getResultCode() == "Ok") {
                        ...
                        $cim_number = User::where("id",
                            $user_id)->update(['cim_no' => $response->getCustomerProfileId()]);

                        $cim_number1 = User::where("id",
                            $user_id)->update(['authorize_net_id' => $response->getCustomerProfileId()]);
...
$cart_ids=$carts->pluck('id');*/
            ...
            $cart_items = CartItems::with('product')->with('cartitemcolor')->with('cartitemimprint')->where('cart_id',
                $cart_id)->get();
            ...
            foreach ($cart_item_ids as $cart_item_id) {
                ...
                $cart_item = CartItems::with('product')->with('cartitemcolor')->with('cartitemimprint')->where('id',
                    $cart_item_id)->first();
...
$cart_ids=$carts->pluck('id');*/
   ...
   return $this->response;*/
    ...
    public function postUsedBillingAdressZero(Request $request)
    {
        ...
        $user_id = $this->getLoginUserId();
        $address = Address::where("user_id", $user_id)->where("is_billing", 1)->where("used_billing",
            1)->update(['used_billing' => 0]);
    ...
    public function postUsedBillingAdress(Request $request)
    {
        ...
        $address_used = Address::where("user_id", $user_id)->where("is_billing", 1)->where("address_id",
            $address_id)->update(['used_billing' => 1]);
    ...
    public function DeleteShipAddress(Request $request)
    {
        ...
        $all_address = Address::where('user_id', $user_id)->where("is_billing", 0)->orderBy('address_id',
            'desc')->get();
...
*/
    ...
    public function index(Request $request)
    {
        $language_id = 1;
        $limit = 20;
        $product_is_stock_collections = Product::query()
            ->where('status_id', 1)
            ->take(20)
            ->get();
        ...
        $latest_products = Product::query()
            ->with([
                'product_translation' => function ($query) use ($language_id) {
                    $query->select('product_translations.product_id',
                        'product_translations.product_name')->where('language_id', $language_id);
                }
            ])
            ->where('status_id', 1)
        ...
        $under_dollar_1 = Product::query()
            ->with([
                'product_prices' => function ($query) {
                    $query->where('per_item_price', '<=', 1);
                }
            ])
            ->with([
                'product_translation' => function ($query) use ($language_id) {
                    $query->select('product_translations.product_id',
                        'product_translations.product_name')
                        ->take(10)
        ...
        $offer_blocks = OfferBlock::query()
            ->where('status_id', 1)
            ->with('default_offer_block_translation')
            ->with([
                'offer_block_translation' => function ($query) use ($language_id) {
                    $query->where('language_id', $language_id);
                }
            ])
            ->get();

        $categories = Category::where("is_main_category", 1)->with('default_category_translation')->with([
            'category_translation' => function ($query) use ($language_id) {
                $query->where('language_id', $language_id)->orderBy('category_name', 'asc');
            }
        ])->where('status_id', '1')->where('is_main_category', 1)->take(12)->get();

        $slider = Slider::where('status_id', 1)->with('default_slider_translation')->with([
            'slider_translation' => function ($query) use ($language_id) {
                $query->where('language_id', $language_id);
            }
        ])->with([
            'product' => function ($query) use ($language_id) {
            ...
            'category' => function ($query) use ($language_id) {
                $query->with('default_category_translation')->with([
                    'category_translation' => function ($query) use ($language_id) {
                        $query->where('language_id', $language_id);
                    }
                ]);
        ...
        $aboutus = HomeAbout::with([
            'aboutus_translation' => function ($query) use ($language_id) {
                $query->where('language_id', $language_id);
            }
...
//$product_ids_from_prices = ProductPrice::whereIn('product_id',$product_ids)->orderBy('per_item_price','asc')->select('product_id')->groupBy('product_id')->pluck('product_id');
        ...
        $product = Product::with('default_product_translation')->with('default_product_translation_full')->with('product_translation')->with('product_translation_full')->where("status_id",
            1)->with('seller')->find($product_id);
        ...
        if ($sku_id != 0) {
            ...
            $recent_view_product = RecentViewProduct::whereDate('created_at', '>=', $current_date)->where('user',
                $id)->where('product_id', $product_id)->first();
            ...
            $sku = Sku::where("product_id", $product_id)->where('parent_variant_id',
                $parent_variant_id)->where('child_variant_id', $child_variant_id)->first();
        ...
        /*Outer If*/
        if ($parent_variant_id == 0) {
            $parent_variant_ids = Sku::where('product_id', $product_id)->where('parent_variant_id', '!=',
                1)->orderBy('parent_variant_id', 'asc')->distinct('parent_variant_id')->pluck('parent_variant_id');

            $parent_variants = VariantOption::with('default_variant_option_translation')->with([
                'variant_option_translation' => function ($query) use ($language_id) {
                    $query->where('language_id', $language_id);
                }
            ...
            if (!$parent_variant_ids->isEmpty()) {
                ...
                $child_variant_ids = Sku::where('product_id', $product_id)->where('parent_variant_id',
                    $parent_variant_id)->where('child_variant_id', '!=', 1)->orderBy('child_variant_id',
                    'asc')->distinct('child_variant_id')->pluck('child_variant_id');
                $child_variants = VariantOption::with('default_variant_option_translation')->with([
                    'variant_option_translation' => function ($query) use ($language_id) {
                        $query->where('language_id', $language_id);
                    }
                ...
                if (!$child_variant_ids->isEmpty()) {
                    $skuTemp = Sku::where("product_id", $product_id)->where('parent_variant_id',
                        $parent_variant_id)->where("child_variant_id", $child_variant_id)->first();
                    if ($skuTemp == "") {
                ...
                $child_variant_ids = Sku::where('product_id', $product_id)->where('child_variant_id', '!=',
                    1)->orderBy('child_variant_id', 'asc')->distinct('child_variant_id')->pluck('child_variant_id');
                $child_variants = VariantOption::with('default_variant_option_translation')->with([
                    'variant_option_translation' => function ($query) use ($language_id) {
                        $query->where('language_id', $language_id);
                ...
                if (!$child_variant_ids->isEmpty()) {
                    $skuTemp = Sku::where("product_id", $product_id)->where("child_variant_id",
                        $child_variant_id)->first();
                    if ($skuTemp == "") {
                        $child_variant_id = $child_variant_ids[0];
                    }
                }
        ...
        $sku = Product::with('product_translation')->with([
            'product_translation_full' => function ($query) use ($language_id) {
                $query->where('language_id', $language_id);
            }
        ])->with([
            'return_policy' => function ($query) use ($language_id) {
                $query->with('default_return_policy_translation')->with([
                    'return_policy_translation' => function ($query) use ($language_id) {
                        $query->where('language_id', $language_id);
                    }
                ]);
            }
            ...
            'parent_variant' => function ($query) use ($language_id) {
                $query->with('default_variant_translation')->with([
                    'variant_translation' => function ($query) use ($language_id) {
                        $query->where('language_id', $language_id);
                    }
                ]);
            }
            ...
            'sku' => function ($query) use ($language_id, $id, $sku_id) {
                $query->with('wishlist')->with([
                    'parent_variant' => function ($query) use ($language_id) {
                        $query->with('default_variant_option_translation')->with([
                            'variant_option_translation' => function ($query) use ($language_id) {
                                $query->where('language_id', $language_id);
        ...
        $reviews = ProductReview::with('images')->where('is_approved', 1)->where('product_id',
            $product_id)->orderBy('review_id', 'desc')->with('user')->take('5')->get();
        ...
        $my_rating = 0;
        $product_review = ProductReview::with('images')->where('is_approved', 1)->where('user_id',
            $id)->where('product_id', $product_id)->first();
        ...
        $category_ids = CategoryProduct::where('category_id', '!=', 1)->where('product_id',
            $product_id)->distinct('category_id')->pluck('category_id');
        if ($category_ids != "[]") {
            $breadcum_category = Category::with('category_translation')->whereIn('category_id', $category_ids)->first();

            $parent_category_id = CategoryHierarchy::where('child_category_id',
                $breadcum_category->category_id)->first();
        ...
        $product_ids = CategoryProduct::whereIn('category_id',
            $category_ids)->distinct('product_id')->take($limit)->pluck('product_id');

        $similar_products = Product::with('sku')->with('brand')->with('reviewCount')->with('default_product_translation')->with([
            'product_translation' => function ($query) use ($language_id) {
                $query->select('product_translations.product_id',
                    'product_translations.product_name')->where('language_id', $language_id);
            }
        ])->whereIn('product_id', $product_ids)->where('status_id', 1)->distinct('product_id')->orderBy('product_id',
            'desc')->where('product_id', '!=', $product_id)->take($limit)->get();


        $cart_count = Cart::where('user', $id)->count();

        $offer_blocks = OfferBlock::where('status_id', 1)->with('default_offer_block_translation')->with([
            'offer_block_translation' => function ($query) use ($language_id) {
                $query->where('language_id', $language_id);
            }
        ])->get();
        ...
        if (isset($_GET['nid'])) {
            ...
            $notification_reads = NotificationRead::where('notification_id', $notification_id)->where('user_id',
                $id)->first();
        ...
        $data = array(
            "images" => $images, "reviews" => $reviews, "my_rating" => $my_rating, 'cart_count' => $cart_count,
            'parent_variants' => $parent_variants, 'child_variants' => $child_variants,
            'parent_variant_id' => $parent_variant_id, 'child_variant_id' => $child_variant_id, 'sku' => $sku,
            'cart' => $cart, "similar_products" => $similar_products, "offer_blocks" => $offer_blocks,
            'wishlist' => $wishlist, "breadcum_category" => $breadcum_category, 'parent_category' => $parent_category
        );
    ...
    public function getAboutUs(Request $request)
    {
        ...
        $aboutus = About::with('default_aboutus_translation')->with([
            'aboutus_translation' => function ($query) use ($language_id) {
                $query->where('language_id', $language_id);
            }
        ])->first();
    ...
    public function getFaq(Request $request)
    {
        ...
        $faq = Faq::with('default_faq_translation')->with([
            'faq_translation' => function ($query) use ($language_id) {
                $query->where('language_id', $language_id);
            }
        ])->first();
    ...
    public function getCart(Request $request)
    {
        ...
        if ($carts != []) {
            $cart_id = $carts->id;
            $cart_items = CartItems::with('product')->with('cartitemcolor')->with('cartitemimprint')->where('cart_id',
                $cart_id)->orderBy('created_at', 'desc')->get();
        } else {
            $cart_items = "[]";
        }
    ...
    public function getTermsConditions(Request $request)
    {
        ...
        $term_conditions = TermCondition::with('default_term_condition_translation')->with([
            'term_condition_translation' => function ($query) use ($language_id) {
                $query->where('language_id', $language_id);
            }
    ...
    public function getPrivacyPolicy(Request $request)
    {
        ...
        $theme_path = $this->getThemePath();
        $policies = Policy::with('default_policy_translation')->with([
            'policy_translation' => function ($query) use ($language_id) {
                $query->where('language_id', $language_id);
            }
        ])->first();
    ...
    public function getshop(Request $request)
    {
        ...
        $products = Product::with('default_product_translation')->with('wishlist')->with('default_product_translation_full')->with([
        ...
        if ($search != "") {
            ...
            $search_content = preg_replace('/[!@#$%^&*()_+\-=\[\]{};\':"\\|,.<>\/?]+/', ',',
                $search); // Removes special chars.
            ...
            foreach ($searches as $search_item) {
                ...
                $category_ids_from_translation = CategoryTranslation::where('category_name', 'like',
                    '%'.$search_item.'%')->pluck('category_id');

                $product_id_from_category = CategoryProduct::whereIn('category_id',
                    $category_ids_from_translation)->distinct()->pluck('product_id');
                ...
                // product
                $product_ids_from_translation = ProductTranslation::where('product_name', 'like',
                    '%'.$search_item.'%')->orWhere('product_id', 'like', '%'.$search_item.'%')->pluck('product_id');
        ...
        $max_product_price = ProductPrice::where('product_id', "!=", '')->where('per_item_price', "!=",
            '')->orderBy('per_item_price', 'desc')->first();
        ...
        $max_product_price_count_from = ProductPrice::where('product_id', "!=", '')->where('count_from', "!=",
            '')->orderBy('count_from', 'desc')->first();
        ...
        if ($min_quantity != "" && $max_quantity != "") {
            ...
            $products = $products->whereHas('product_prices', function ($query) use ($min_quantity, $max_quantity) {
                $query->where('count_from', ">=", $min_quantity)->where('count_from', "<=",
                    $max_quantity)->orderBy('count_from', 'asc');
        ...
        $qty_rangeLimits = array(
            $min_quantity_value, $max_qty_1, $max_qty_2, $max_qty_3, $max_qty_4, $max_quantity_value
        );
        $qty_ranges = array();
        ...
        if ($orderby != "") {
            ...
            if ($orderby == "LowToHighPrice") {
                ...
                $product_ids = $products->pluck('product_id');
                $product_ids_from_prices = ProductPrice::whereIn('product_id', $product_ids)->orderBy('per_item_price',
                    'asc')->select('product_id')->pluck('product_id');
                $product_ids_from_prices = $product_ids_from_prices->toArray();
                $product_ids_ordered = implode(',', $product_ids_from_prices);
                $products = $products->whereIn('product_id',
                    $product_ids_from_prices)->orderByRaw("FIELD(product_id, $product_ids_ordered)");
            ...
            if ($orderby == "HighToLowPrice") {
                ...
                $product_ids = $products->pluck('product_id');
                $product_ids_from_prices = ProductPrice::whereIn('product_id', $product_ids)->orderBy('per_item_price',
                    'desc')->select('product_id')->pluck('product_id');
                $product_ids_from_prices = $product_ids_from_prices->toArray();
                $product_ids_ordered = implode(',', $product_ids_from_prices);
                $products = $products->whereIn('product_id',
                    $product_ids_from_prices)->orderByRaw("FIELD(product_id, $product_ids_ordered)");
        ...
        if ($shop_cat_id != "") {
            $child_category_ids = CategoryHierarchy::where('parent_category_id',
                $shop_cat_id)->pluck('child_category_id');
            $shop_category_ids = [];
...
// })->get();
        ...
        return view('superior.category')->with('products', $products)->with('categories',
            $categories)->with('category_ids', $category_ids)->with('colors', $colors)->with('color_ids',
            $color_ids)->with('ranges', $ranges)->with('max_prices', $max_prices)->with('min_prices',
            $min_prices)->with('orderby', $orderby)->with('page', $page)->with('search', $search)->with('cat_id',
            $cat_id)->with('pagi_num', $pagi_num)->with('shop_cat_id', $shop_cat_id)->with('menu_products',
            $menu_products)->with('user_wishlists', $user_wishlists)->with('wishlist',
            $wishlist)->with('max_quantities', $max_quantities)->with('min_quantities',
            $min_quantities)->with('qty_ranges', $qty_ranges)->with('under_dollar_1', $under_dollar_1);
    }
    ...
    public function getMyProfile()
    {
        ...
        if ($user != "") {
            ...
            $billing_addresses = Address::where("user_id", $user->id)->where('is_billing',
                1)->with('city')->with('state')->with('country')->with('user')->get();
            $shipping_addresses = Address::where("user_id", $user->id)->where('is_billing',
                0)->with('city')->with('state')->with('country')->with('user')->get();

            // use is shipping
            $user_address = Address::where('user_id', $user->id)->where("is_shipping", 1)->where("is_billing",
                0)->with('city')->with('state')->with('country')->with('user')->first();
            if ($user_address == "") {
                $user_address = Address::where('user_id', $user->id)->where("is_default_add", 1)->where("is_billing",
                    0)->with('city')->with('state')->with('country')->with('user')->first();
            }

            if ($user == "") {
                $user = Address::where('user_id', $user_id)->where("is_billing",
                    0)->with('city')->with('state')->with('country')->with('user')->first();
            }
            ...
            $order_history = Order::where('is_reorder', 0)->where('user_id',
                $user->id)->with('orderitems')->orderBy('id', 'desc')->get();
            ...
            $art_proofs = ArtProof::where('user_id', $user_id)->where('approved',
                2)->with('orderitem')->with('user')->get();
...
// })->get();
            ...
            $reorder_lists = Order::where('user_id', $user->id)->where('is_reorder',
                1)->with('orderitem')->orderBy('id', 'desc')->get();
            ...
            return view('superior.my-account-profile')->with('my_account_profile', $my_account_profile)->with('user',
                $user)->with('wishlists', $wishlists)->with('wishlist', $wishlist)->with('billing_addresses',
                $billing_addresses)->with('shipping_addresses', $shipping_addresses)->with('data',
                $data)->with('order_history', $order_history)->with('user_address', $user_address)->with('art_proofs',
                $art_proofs)->with('reorder_lists', $reorder_lists)->with('today_current_date',
                $today_current_date)->with('save_cart_items', $save_cart_items);
        } else {
            return redirect('/');
        }
    ...
    public function postMyAccBillAddressAdd(Request $request)
    {
        ...
        $address_new = Address::where('address_id',
            $address->address_id)->with('city')->with('state')->with('country')->with('user')->first();
    ...
    public function postMyAccShippAddressAdd(Request $request)
    {
        ...
        if ($request->is_default_add == "true") {
            ...
            $before_default_shipping_address = Address::where('user_id', $user_id)->where("is_shipping",
                1)->where("is_billing", 0)->with('city')->with('state')->with('country')->with('user')->first();
            if ($before_default_shipping_address != "") {
                $view2 = View::make('superior.unMakeDefaultShippingAddress')->with('address',
                    $before_default_shipping_address);
                $sections2 = $view2->renderSections();
            } else {
        ...
        $address_new = Address::where('address_id',
            $address->address_id)->with('city')->with('state')->with('country')->first();
        ...
        $data = array(
            "sections" => $sections['addNewShipAddress'], 'sections2' => $sections2['unMakeDefaultShippAddress'],
            'is_default_add' => $is_default_add
        );
    ...
    public function postMyAccShipAddressEdit(Request $request)
    {
        ...
        if ($request->is_default_add == "true") {
            ...
            if ($before_default_shipping_address != "") {
                ...
                $address = Address::where("user_id", $user_id)->where("is_shipping", 1)->update(['is_shipping' => 0]);
                $before_default_shipping_address = Address::where('address_id',
                    $before_default_shipping_address_id)->with('city')->with('state')->with('country')->with('user')->first();
                if ($before_default_shipping_address != "") {
                    $view2 = View::make('superior.unMakeDefaultShippingAddress')->with('address',
                        $before_default_shipping_address);
                    $sections2 = $view2->renderSections();
        ...
        $address = Address::where('address_id',
            $new_address_id)->with('city')->with('state')->with('country')->with('user')->first();
        ...
        $data = array(
            "sections" => $sections['editShippAddressMyAcc'], "sections2" => $sections2['unMakeDefaultShippAddress'],
            "address_id" => $address_id, 'is_default_add' => $is_default_add,
            'before_make_default_address_id' => $before_make_default_address_id
        );
    ...
    public function postMyAccShippAddressMakeDefault(Request $request)
    {
        ...
        $address_id = $request->address_id;
        $address = Address::where('address_id',
            $address_id)->with('city')->with('state')->with('country')->with('user')->first();
        if ($address != "") {
            $address = Address::find($address_id);
            ...
            $data = array(
                "sections" => $sections['makeDefaultShippAddress'],
                'sections2' => $sections2['unMakeDefaultShippAddress'], "address_id" => $address_id
            );
...
*/
    ...
    public function getCheckout(Request $request)
    {
        ...
        if ($carts != "") {
            ...
            $total_carts = CartItems::with('product')->with('cartitemcolor')->with('cartitemimprint')->where('cart_id',
                $cart_id)->orderBy('created_at', 'desc')->get();
        ...
        if ($carts != "") {
            $discount_data = Discount::with('discount_types')->with('discount_apply')->with('discount_customer_apply')->where('discount_id',
                $carts->discount_id)->first();
        ...
        $data = array(
            "all_billing_address" => $all_billing_address, "all_address" => $all_address, "user" => $user,
            "product_total" => $product_total, "order_total" => $order_total, "shipping_cost" => $shipping_cost,
            "discount" => $discount, "disc_code" => $disc_code, "Allstates" => $Allstates, "Allcity" => $Allcity
        );
    ...
    public function getMatchQuantity(Request $request)
    {
        ...
        $Product_prices = ProductPrice::where('product_id', $product_id)->where('count_from', '<=',
            $quantity_cart)->pluck('count_from');
        ...
        if ($closest !== null && $closest != []) {
            $matches_quantitydata = ProductPrice::where('product_id', $product_id)->where('count_from',
                $closest)->first();
            $per_item_price = $matches_quantitydata->per_item_sale_price;
            $subtotal = $per_item_price * $quantity_cart;
        ...
        if ($product_option_prices_ids != "") {
            foreach ($product_option_prices_ids as $option_prices_id) {
                if ($option_prices_id != "" && $option_prices_id != 0) {
                    ...
                    $Product_suboption_prices = ProductSubOptionPrices::where('product_sub_option_id',
                        $option_prices_id)->where('quantity', '<=', $quantity_cart)->pluck('quantity');
                    ...
                    if ($Product_suboption_prices != "[]") {
                        ...
                        $matches_quantitydata = ProductSubOptionPrices::where('product_sub_option_id',
                            $option_prices_id)->where('quantity', $closest)->first();
            ...
            if ($request->imprint_ids != "") {
                ...
                if ($imprint_ids_col != "[]") {
                    ...
                    for ($i = 0; $i < count($imprint_ids_col); $i++) {
                        ...
                        $imprint = Imprint::where('id',
                            $imprint_id)->with('imprint_price')->with('imprint_colors')->first();
...
*/
    ...
    public function getShippingCharge(Request $request)
    {
        ...
        if (isset($array_data['SOAPENVBody']['RateReply']['RateReplyDetails']['RatedShipmentDetails'])) {
            ...
            $fdate = date('Y-m-d');
            $tdate = date('Y-m-d',
                strtotime($array_data['SOAPENVBody']['RateReply']['RateReplyDetails']['CommitDetails']['CommitTimestamp']));
         ...
         ';
            $show .= '
               <tr>
                  ...
                  <td>'.str_replace('_', ' ',
                    $array_data['SOAPENVBody']['RateReply']['RateReplyDetails']['ServiceType']).'</td>
