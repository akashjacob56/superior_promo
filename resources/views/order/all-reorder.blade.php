              ...
              <div class="card">
                ...
                <!-- <div class="card-block">
                  ...
                  @foreach($production_stages as $product_stage)
                    <div class="row">
                      <div class="col-12">
                      <a href="{{$base_url}}/admin/order/all?stage_id={{$product_stage->id}}">
                        {{$product_stage->name}}
                    ...
                    <table id="footer-search" class="table nowrap">
                      <thead>
                        <tr>
                          ...
                          <td></td>
                          <td>@if($order->user!="") {{$order->user->name}}(<a href="{{$base_url}}/admin/customer/{{$order->user_id}}">{{$order->user_id}}</a>) @endif</td>
                          <td></td>
                          <td></td>
                          <td></td>
                      ...
                      <tbody>
                        ...
                        <tr class="{{$is_important_color}} selection_excla_sign_{{$orderitem->id}}">
                            <td><a href="javascript:void(0);">{{$orderitem->id}}</a></td>
                            ...
                            <td>
                              ...
                              <div class="order_item_vendor_detail_{{$orderitem->id}} hidden" style="margin-top: 5px;">
                                ...
                                <img style="width: 50px;height: 60px;" src="{{$base_url}}/storage/app/{{$orderitem->product->product_image}}"><br>
                                @endif
                          ...
                          <table class="table table-bordered" id="stage_history_table_{{$orderitem->id}}">
                            <tbody class="stage-historydata stage-historydata-{{$orderitem->id}}">
                              ...
                              @if($orderitem->order_item_stage!="[]")
                                ...
                                <tr>
                                  <!-- <td>Jan/14/2022 07:17</td> -->
                                  <td><?php echo date("m-d-Y", strtotime($order_item_stage->created_at)); ?></td>
    ...
    <tbody class="tbody_{{$orderitem->id}}">
      ...
      @if($orderitem->order_notes!="[]")
        @foreach($orderitem->order_notes as $order_note)
          ...
          <tr>
            <td><?php echo date("m-d-Y", strtotime($order_note->created_at)); ?></td>
            <td>
              @if($order_note->user!="")
...
<h6 class="modal-title pt-1 pb-2">Art Proofs</h6>
  ...
  <tbody class="art_proof_management_tbody_{{$orderitem->id}}">
    ...
    <tr>
      <td>{{$artproof->id}}</td>
      <td><?php echo date("m-d-Y", strtotime($artproof->created_at)); ?></td>
      <td>
        @if($artproof->user!="")
        {{$artproof->user->name}}
        @endif
      ...
      <td class="art_proof_mng_path_td">{{$artproof->path}}</td>
      ...
      <td><?php echo date("m-d-Y", strtotime($artproof->updated_at)); ?></td>
    </tr>
    @endforeach
    @endif
  </tbody>
...
<!-- <script type="text/javascript">
  ...
  //     return 'fghf';
  //     // return '<td><a href="{{$base_url}}/admin/artwork/all?order_item_id='+row.id+'">Art Proof</a></td>';
  ...
  // },
      ...
      //   "mRender":function(data,type,row){
      //     return 'lsdfjl';
      //     // return '<a href="{{$base_url}}/admin/product/'+row.orderitems.product.product_id+'"><button class="btn btn-primary waves-effect waves-light js-programmatic-disable data-table-button">Product Detail</button></a>';
      //   }
      // },
      // {
...
<script type="text/javascript">
  $(document).ready(function(){
      ...
      $('i.exclamation_click_order').on('click',function (event){
        ...
        $.ajax({
            type:"POST",
            url:"{{$base_url}}/admin/order/order-item-important",
...
<script type="text/javascript">
  $(document).ready(function(){
    $('.art_proof_management').on('click','.btn-art-proof',function(){
        ...
        $.ajax({
            ...
            success: function(data){
              ...
              console.log(data['art_proof']);
                $('.art_proof_management_tbody_'+data['art_proof']["order_item_id"]).append('<tr><td>'+data['art_proof']["id"]+'</td><td>'+data["created_at"]+'</td><td>'+data['art_proof']["user"]["name"]+'</td><td class="art_proof_mng_path_td">'+data['art_proof']["path"]+'</td><td>'+data['art_proof']["note"]+'</td><td>'+customer_note+'</td><td>'+approve+'</td><td>'+data['updated_at']+'</td></tr>');
...
<script type="text/javascript">
  $(document).ready(function(){
      $('.select_vendor').on('change',function(){
          ...
          $.ajax({
            method: "POST",
            url: "{{$base_url}}/admin/order/select-vendor",
            dataType: 'json',
            cache: false,
            contentType: false,
            processData: false,
