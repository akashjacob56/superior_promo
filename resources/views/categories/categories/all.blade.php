                  ...
                  @foreach($menu_categories as $category)
                      ...
                      @if(count($category->child_categories)>0)
                          <li class="parent_content" id="{{$category->category_id}}">
                            <a  class="" href="javascript:void(0);">{{$category_translation->category_name}} &nbsp;</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa fa-pencil category_edit_{{$category->category_id}} "></i>&nbsp;<i class="fa fa-trash category_inactive_{{$category->category_id}}"></i>
                                  ...
                                  @foreach($category->child_categories as $child_category)
                                        ...
                                        @if(count($child_category->sub_child_categories)>0)
                                               ...
                                               <li id="{{$child_category_translation->category_id}}" class="child_content" style="flex: 0 0 98%;margin-left: 2%;">
                                                  <a  class="" href="#">{{$child_category_translation->category_name}}</a>&nbsp;<i class="fa fa-pencil  child_category_edit_{{$child_category_translation->category_id}}"></i>&nbsp;<i class="fa fa-trash child_category_inactive_{{$child_category_translation->category_id}}"></i>
                                                  <ul class="menu vertical sublevel-2">
                                                    @foreach($child_category->sub_child_categories as $sub_child_category)
                                                        ...
                                                        <li class="subchild_content" id="{{$sub_child_category->category->category_id}}" class="sub_child_content" style="flex: 0 0 96%;margin-left: 4%;"><a  class="" href="#">{{$sub_child_category_translation->category_name}}</a> &nbsp;<i class="fa fa-pencil sub_child_category_edit_{{$sub_child_category->category->category_id}}"></i>&nbsp;<i class="fa fa-trash sub_child_category_inactive_{{$sub_child_category->category->category_id}}"></i>
                                                        </li>
                      ...
                      @else
                          ...
                          <li id="{{$category->category_id}}" class="parent_content">
                            <a  class="" class="subitem" href="#">{{$category_translation->category_name}}</a> &nbsp;<i class="fa fa-pencil category_edit_{{$category->category_id}}"></i>&nbsp;<i class="fa fa-trash category_inactive_{{$category->category_id}}"></i>
                          </li>
        ...
        $( ".parent_content" ).droppable({
            ...
            drop: function( event, ui ) {
                ...
                $.ajax({
                  type: 'post',
                  url: '/admin/category/drag-drop',
                  data: {"_token": "{{ csrf_token() }}",'child_category_id':child_id,'new_parent_category_id':parent_id},
                  success: function (result){
                    location.reload();
                  },
                  error: function (xhr, textStatus, errorThrown) { alert(textStatus + ':' + errorThrown); }
