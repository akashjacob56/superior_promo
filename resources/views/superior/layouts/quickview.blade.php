...
<script type="text/javascript">
  var is_inventory='{{$appearance->is_inventory}}';
  ...
  if(product.default_product_translation.description!=null){
    string=product.default_product_translation.description;
       ...
       var barcode="";
        ...
        if(product.sku.my_price>0){
          if(product.sku.market_price-product.sku.my_price >= 0)
          {
            ...
            offer='<span class="text-primary product-price m-0">{{$appearance_translation->currency}}'+product.sku.my_price+'</span> <span class="done-task txt-muted">{{$appearance_translation->currency}}'+product.sku.market_price+'</span> <span class="offer text-primary"> <strong> '+offer+' % @lang("product.off") </strong> </span>';
          }else{
            offer='<span class="text-primary product-price">{{$appearance_translation->currency}}'+product.sku.my_price+'</span>';
          }
        }
       ...
       /* else{
        ...
        if(product.track_inventory==1 && is_inventory==1){
