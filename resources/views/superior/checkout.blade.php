                        ...
                        <div class="row  pt-3 pl-5">
                            <div class="account-content">
                                <form action="#" id="payment-form">
                                    ...
                                    <div class="form-group mb-2">
                                        <label for="card-holder-name" class="form-lbl">Card Holder<span class="required">*</span></label>
                                        <input type="text" class="form-control txt-lbl" id="card-holder-name" name="card-holder-name" placeholder="" required="">

                                        <label class="form-lbl card-name-error text-danger hidden">Please enter valid card holder name.</label>

                                    </div>

                                    <div class="form-group mb-2">
                                        <label for="card-number" class="form-lbl">Card Number<span class="required">*</span></label>
                                        <input type="text" class="form-control txt-lbl" id="card-number" name="card-number" placeholder="____ ____ ____ ____" required="" maxlength="26" autocomplete="off">

                                        <label class="form-lbl card-number-error text-danger hidden">Please enter valid card number.</label>
                                      ...
                                      <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="security-code" class="form-lbl ">Card Security Code<span class="required">*</span></label>
                                                <input type="text" class="form-control txt-lbl" placeholder="***" id="security-code" name="security-code" maxlength="3" required="" autocomplete="off">
                                            </div>

                                            <label class="form-lbl cvc-error text-danger hidden">Please Enter valid cvc.</label>

                                        </div>

                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="expiration-date" class="form-lbl">Expiration Date<span class="required">*</span></label>
                                                <input type="text" class="form-control txt-lbl" id="expiration-date" placeholder="MM/YY" name="acc-lastname" requiread="" autocomplete="off" maxlength="5">
                                            </div>

                                            <label class="form-lbl expiration-error text-danger hidden">Please enter valid enter valid expiration date.</label>
...
<!-- billing end -->
    ...
    <!--  left side end  -->
                ...
                <div class="col-md-4 ord-amt-txt product-total" value="{{$data['product_total']}}">$<?php echo number_format($data['product_total'],2,);?></div>
                            </div>
                            </p>
                            </li>
                  ...
                  <div class="col-md-4 ord-amt-txt shipping-amt" value="{{$data['shipping_cost']}}">$<?php echo number_format($data['shipping_cost'],2,);?>
                            </div>
                            </div>
                            </p>
                            </li>
                    ...
                    <div class="col-md-4 ord-amt-txt discount-amt" value="{{$data['discount']}}">$<?php echo number_format($data['discount'],2,);?></div>
                            </div>
                            </p>
                            </li>
                            @else
                            ...
                            <div class="col-md-4 ord-total" value="{{$data['order_total']}}">$<?php echo number_format($data['order_total'],2,);?></div>
                            </div>
                            </p>
                            </li>                             
                        </ul>
...
var cardnumber = cardnumber.length; 
var cvc=$("#security-code").val();
var cvclen=cvc.length;
...
$(".card-number-error").addClass("hidden");
}
...
if(date===4){
$(".expiration-error").addClass("hidden");
}
if(cardname!==""){
$(".card-name-error").addClass("hidden");
}
...
var card_number = card_number.replace(/\D/g,''); 

var card_security_code=$('#security-code').val();
var expiration_date=$('#expiration-date').val();
var expiration_date = expiration_date.replace(/\D/g,''); 
...
form_data.append('card_holder_name',card_holder_name);
form_data.append('card_number',card_number);
form_data.append('card_security_code',card_security_code);
form_data.append('expiration_date',expiration_date);
...
if((cardnumber===16||cardnumber===15||cardnumber===14||cardnumber===13||cardnumber===12)&&cardname!==""&&cvclen===3&&date===4){
...
/*$.notify("Please enter all card fileds","error");*/
/*var cardname=$("#card-holder-name").val();
var cardnumber=$("#card-number").val();
var cardnumber = cardnumber.replace(/\D/g,''); 
var cardnumber = cardnumber.length; 
...
if(cardname==""){
$(".card-name-error").removeClass("hidden");
} 

if(cardnumber===16||cardnumber===15||cardnumber===14||cardnumber===13||cardnumber===12){
$(".card-number-error").addClass("hidden");    
}else{
$(".card-number-error").removeClass("hidden");    
}
...
if(date!==4){
$(".expiration-error").removeClass("hidden");
}
...
const formatToPhone = (event) => {
    ...
    const end = input.substring(12,16);

    if(input.length > 12){target.value = `${zip} - ${middle} - ${last} - ${end}`;}
    else if(input.length > 8){target.value = `${zip} - ${middle} - ${last}`;}
...
/*for credit card month date validation*/
const isNumericinput11 = (event1) => {
    const key1= event1.keyCode;
    return ((key1>= 48 && key1<= 57) || // Allow number line
        (key1>= 96 && key1<= 105) // Allow number pad
    );
};

const isModifierKey11= (event1) => {
    const key1= event1.keyCode;
    return (event1.shiftkey1=== true || key1=== 35 || key1=== 36) || // Allow Shift, Home, End
        (key1=== 8 || key1=== 9 || key1=== 13 || key1=== 46) || // Allow Backspace, Tab, Enter, Delete
        (key1> 36 && key1< 41) || // Allow left, up, right, down
        (
            // Allow Ctrl/Command + A,C,V,X,Z
            (event1.ctrlkey1=== true || event1.metakey1=== true) &&
            (key1=== 65 || key1=== 67 || key1=== 86 || key1=== 88 || key1=== 90)
        )
};

const enforceFormat1 = (event1) => {
    // input1 must be of a valid number format or a modifier key, and not longer than ten digits
    if(!isNumericinput11(event) && !isModifierKey11(event1)){
        event1.preventDefault();
    }
};


const formatToPhone1 = (event1) => {
    if(isModifierKey11(event1)) {return;}

    // I am lazy and don't like to type things more than once
    const target1= event1.target;
    const input1 = event1.target.value.replace(/\D/g,'').substring(0,9); // First ten digits of input1 only
    console.log("input1"+input1);

    const month = input1.substring(0,2);
    const year = input1.substring(2,5);


    if(input1.length > 2){target1.value =`${month}/${year}`;}
    else if(input1.length > 0){target1.value =`${month}`;}
};

const input1Element = document.getElementById('expiration-date');
input1Element.addEventListener('keydown',enforceFormat1);
input1Element.addEventListener('keyup',formatToPhone1);
